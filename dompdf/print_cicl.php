...
$sql = "SELECT * FROM client c INNER JOIN client_child_details s, interview i WHERE i.interview_id = s.interview_id && s.client_id = $client_id && c.client_id = $client_id && i.client_id = $client_id && s.client_child_detail_id = $count_no";
...
$when = date("M d, Y", strtotime($row['when_happened']));
$whom = $row['to_whom'];
$service_rendered = $row['services_rendered'];
$referred = $row['reffered_by'];
$educ = $row['educ_attainment'];
$counterno = $row['client_child_detail_id'];
...
input[type=radio]:before { font-family: DejaVu Sans; }
	...
	<center style='color: blue;'>
		<br><img src='images/dswd-logo.jpg' style='margin-right: 13%;' alt='images/dswd-logo' width='100' height='100' align='right' style='display: block; margin-left: 1% ; '>
			...
			<h4 style='margin:0; color: black'><b>Children in Conflict with the law Intake</b></h4>
	</center>
	...
	<input style='width: 265px; border: none; border-bottom: 1px solid black; text-align: center;' value='$referred' type='text' />
	<br>
	<input style='margin-left: 4.5%; width: 130px; border: none;' value='Type of Offense' type='text' /><b>:</b>
	<input style='width: 185px; border: none; border-bottom: 1px solid black; text-align: center;' value=".$row['type_of_offense']." type='text' />
	<input style='width: 75px; border: none;' type='text' value='Committed:' />
	<input style='width: 270px; border: none; border-bottom: 1px solid black; text-align: center;' value=".$row['committed']." type='text' />
	<br>
	...
	<input style='width: 270px; border: none; border-bottom: 1px solid black; text-align: center;' value='$whom' type='text' />
	<input style='width: 140px; border: none;' type='text' value='Service Rendered:' />
	<input style='width: 120px; border: none; border-bottom: 1px solid black; text-align: center;' value='$service_rendered' type='text' />
	<br>
	<input style='margin-left: 4.5%; width: 130px; border: none;' type='text' value='Disposition' /><b>:</b><br>
	<textarea style='margin-left: 4.5%; margin-right: 4.5%; font-family: 'Times New Roman', Times, serif;'>&nbsp;&nbsp;&nbsp;".$row['disposition']."</textarea>
	<br>
